...
function toggleAccountsDataTo(selectedAccount) {
    var appears = false;
    var accountsData = document.getElementsByClassName('accountsTo');
    for(var i = 0; i < accountsData.length; i++) {
        display = accountsData[i].style.display;
        if (display=="block") {
            appears = true;
    ...
    if (!appears) { // if accounts data is hidden
        var selectAmount = document.getElementById('selectAccountTo');
        selectAmount.innerHTML = '';
        // show accounts data
        for(var i = 0; i < accountsData.length; i++) {
            accountsData[i].className = accountsData[i].className.replace
...
function loadAccounts() {
    ...
    $.when(getAccounts()).done(function(accounts) {
                               $.each(accounts, function(i, account){
                                      ...
                                      myDivTo = '<a id="account'+account.accountId+'" onClick="toggleAccountsDataTo(\'ACCOUNT '+account.accountNumber.substr(account.accountNumber.length - 5)+'\')" class="list-group-item">'+
                                      '<table class="accountsTo">'+
                                      '<tr>'+
                                      '<td style=padding:10px">'+account.accountType+'</td>'+
                                      '<td style="text-align:right;"><font color="#000000">CAD $'+
                                      parseFloat(Math.round(account.accountBalance * 100) / 100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")+'</font></td>'+
